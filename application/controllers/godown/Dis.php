...
class Dis extends MX_Controller {
	...
	public function index(){
		...
		$data['dockets']   = $this->FertilizerModel->f_get_particulars('td_docket_no a,md_users b',$select,$where,0);
		$this->load->view('common/header');
		$this->load->view('docket/docket',$data);
	...
	public function docdetail(){
		if($_SERVER['REQUEST_METHOD']=="POST"){
		  ...
		  $view = $this->load->view('docket/documentdetail',$data);
		  return $view;
		}
	}
	...
	public function forward(){
		...
		$this->load->view('common/header');
		$this->load->view('docket/forward',$data);
		$this->load->view('common/footer');
	}
	...
	public function docket_detail(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST"){
			...
			//if(count($query) > 0){
				...
				$view = $this->load->view('docket/documentblock',$data);
				return $view;
			// }else{
			// 	return 0;
			// }
	...
	public function searchdoc(){
        if($_SERVER['REQUEST_METHOD']=="POST"){
			...
			$this->load->view('common/header');
			$this->load->view('docket/search_doc',$data);
			$this->load->view('common/footer');
	...
	public function docket_detaillist(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST"){
			...
			$query = $this->db->get_where('td_document', array('docket_no =' => $docket_no))->result();
			   ...
			   $data['dockets']   = $this->master->f_get_particulars('td_docket_no a,md_users b',$select,$where,0);
			   $view = $this->load->view('docket/document_detailist',$data);
			   return $view;
